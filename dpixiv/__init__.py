...
class DPixivIllusts:
    ...
    def load_ids_from_pages(self, url, parser, page, from_page, to_page, step_count, params={}):
        if page:
    ...
    def bookmarks(self, page=None, from_page=1, to_page=1000000, step_count=10): #Get bookmarks of user
        ...
        return self.load_ids_from_pages(url, self.__bookmarks_parser, page, from_page, to_page, step_count)
    ...
    def new_work_following(self, page=None, from_page=1, to_page=1000000, step_count=10):
        ...
        return self.load_ids_from_pages(url, self.__parser_where_data_items, page, from_page, to_page, step_count)
    ...
    def search(self, word, page=None, from_page=1, to_page=1000000, step_count=10):
        ...
        params = {'word': word}
        return self.load_ids_from_pages(url, self.__parser_where_data_items, page, from_page, to_page, step_count, params)

    def add_tag(self, pic_id, tag, token):
        res = json.loads(self.post('https://www.pixiv.net/ajax/tags/illust/{}/{}/add'.format(pic_id, tag), csrf_token=token))
        return res

    def del_tag(self, pic_id, tag, token):
        res = json.loads(self.post('https://www.pixiv.net/ajax/tags/illust/{}/{}/delete'.format(pic_id, tag), csrf_token=token))
    ...
    def __parse_info(self, page, token):
        ...
        if find_json:
            ...
            result = result['preload']['illust'] if 'preload' in result and 'illust' in result['preload'] else None
            if result and token:
                token = find_token.search(page)
                result[next(iter(result))]['token'] = token[1] if token else None
            return result
    ...
    def info_packs(self, illusts, token=False): #Return all info; get list of ids
        pages = self.__get_list_with_id([{'id': one, 'url': 'https://www.pixiv.net/member_illust.php', 'params': {'mode': 'medium', 'illust_id': one}} for one in illusts])
        return [self.__parse_info(pages[ilust], token) for ilust in illusts]
